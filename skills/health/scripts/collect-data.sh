...
# NOTE: pipefail intentionally omitted. Several summaries end a pipeline in `head`
# (skill descriptions, allowedTools, conversation files). When the producer has more
# lines than `head` reads, it takes SIGPIPE on the next write; under pipefail+errexit
# that 141 would abort the whole collection (observed with 280+ installed skills).
# errexit + nounset are kept; any coverage gap degrades to "(none)"/"(unavailable)".
set -eu
