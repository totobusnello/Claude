...
## 2026-07-12 — CICLO 23 — Claim 0026 endurecido: 2ª implementação + REV-0013 (Codex) SUSTENTADA; 1 finding corrigido

**Contexto:** Luiz pediu tarefa executável/review na paralela dos pilotos. Três frentes: (1) dry-run do `analyze_pilot.py` nos parciais das 3 frentes — PASSOU (interim: estrato aleatório concentrado em opt≥10, censura dominante em k=9/10, mediana ~55min/classe decidida); (2) **2ª implementação independente do tree n=4** (`tree_gap_n4_v2.py`, Bellman fixed-point global com as 4 polaridades explícitas — estrutura deliberadamente distinta do BFS por camadas): **65.536/65.536 células idênticas** à v1, ponto fixo em 6 sweeps; (3) **REV-0013** (Codex/gpt-5.6-sol, via rev.sh, background): re-derivação independente ANTES de ler nosso código — distribuição EXATA, mesmos 3 max-gap, NPN-canonicidade re-verificada, Khrapchenko com A/B explícitos (|E|=32 ⟹ 16 folhas ⟹ ≥15 portas). **SUSTENTADA.**

**Finding da REV-0013 (real, corrigido):** o `npn4_opt_aig.csv` mantinha `improved_ub` em 0x1669/0x166b — metadado obsoleto (os DRATs k=9 dos claims 0022/0023 estabelecem opt=10 exato; o CSV nunca foi atualizado quando as sondas fecharam) — e o join do censo não checava a coluna `status`. Correções: CSV atualizado (lastro registrado aqui, não é reescrita de histórico — o estado da época segue documentado no EXP-PROBE-0001) + assert no join exigindo catálogo 100% exact; censo re-executado, resultado inalterado.
